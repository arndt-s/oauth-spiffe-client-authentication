...
Client authentication using JWT-SVIDs have the same security considerations as described in {{RFC6749}} and {{RFC7521}}.

Client authentication using X509-SVIDs have the same security considerations as described in {{RFC8705}}. The validation rules in section 3.2 protect against a Oauth2 token being issued (or being issued incorrectly) to a client that did not present an appropriate X509-SVID. 

In both setups, the issues described in Section 5.2 above include the threat that an authorization server may have the incorrect
